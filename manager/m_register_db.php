    ...
    if (empty($user_status)) {
        ...
        try {
            ...
            if ($row['username'] == $username) {
                ...
                VALUES(:user_status,:firstname,:lastname,:username,:password,:address,:gender,:tel,:email,:img_profile)");

                move_uploaded_file($_FILES['img_profile']['tmp_name'],"../pic_profile/".$filename);
 
                $insert_stmt->bindParam(":user_status", $user_status, PDO::PARAM_STR);
                ...
                $insert_stmt->execute();
                $_SESSION['success'] = 'ลงทะเบียนสำเร็จ!! <a class="h4" href="../login.php">คลิกที่นี่เพื่อเข้าสู่ระบบ</a> ';
                header("location:m_reg.php");
            } else {
